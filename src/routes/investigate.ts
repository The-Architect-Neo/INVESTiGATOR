...
import { alertCanaryTripped } from '../utils/canary'

// Alias to avoid collision with the global Fetch API Response type
...
// ──────────────────────────────────────────────────────────────────
const INVESTIGATOR_INPUT_CANARIES: Record<string, string> = {
...
function checkInputCanary(query: string, type: string, req: Request): void {
  const normalised = query.toLowerCase().trim()
  const canaryRef = INVESTIGATOR_INPUT_CANARIES[normalised]
  if (canaryRef) {
    alertCanaryTripped(canaryRef, {
      path: req.path,
      type,
      ip: req.ip,
...
// Input validation
// ──────────────────────────────────────────────────────────────────
const QUERY_TYPES = ['company', 'fca', 'domain', 'ip', 'postcode', 'phone', 'email'] as const
...
// ──────────────────────────────────────────────────────────────────
async function lookupCompaniesHouse(query: string) {
...
// ──────────────────────────────────────────────────────────────────
async function lookupCompanyOfficers(companyNumber: string) {
  const key = process.env.COMPANIES_HOUSE_API_KEY
  if (!key) return null
...
// ──────────────────────────────────────────────────────────────────
async function lookupFCA(query: string) {
  try {
    ...
    const json = await res.json() as {
      Data?: Array<{
        ...
        'Current Authorisation Status'?: string
        'Firm Type'?: string
      }>
    }
...
// ──────────────────────────────────────────────────────────────────
async function lookupFCAWarning(query: string) {
  try {
...
// ──────────────────────────────────────────────────────────────────
async function lookupDisqualifiedDirectors(query: string) {
  const key = process.env.COMPANIES_HOUSE_API_KEY
  if (!key) {
    return {
  ...
  try {
    ...
    const json = await res.json() as {
      ...
      items?: Array<{
        ...
        disqualifications?: Array<{
          ...
          reason?: { description_identifier?: string }
        }>
      }>
    }
    ...
    for (const person of items) {
      const activeDisq = (person.disqualifications ?? []).filter(d => {
        if (!d.disqualified_until) return true
...
// ──────────────────────────────────────────────────────────────────
async function lookupCompanyCharges(companyNumber: string) {
  const key = process.env.COMPANIES_HOUSE_API_KEY
  if (!key) return null
...
// ──────────────────────────────────────────────────────────────────
async function lookupFilingHistory(companyNumber: string) {
  const key = process.env.COMPANIES_HOUSE_API_KEY
  if (!key) return null
...
// ──────────────────────────────────────────────────────────────────
async function lookupCharityCommission(query: string) {
  try {
...
async function lookupRDAP(domain: string) {
  try {
    ...
    if (registered) {
      ...
      if (ageDays < 30) flags.push(`RECENTLY_REGISTERED: Domain registered ${Math.round(ageDays)} days ago — common fraud indicator`)
    }
...
// ──────────────────────────────────────────────────────────────────
async function lookupVirusTotal(query: string, type: 'domain' | 'ip') {
...
// ──────────────────────────────────────────────────────────────────
async function lookupURLScan(domain: string) {
  try {
    const clean = domain.replace(/^https?:\/\//i, '').split('/')[0].toLowerCase()
    ...
    if (searchRes.ok) {
      const searchJson = await searchRes.json() as {
        ...
        results?: Array<{
          ...
          page?: { domain?: string; ip?: string; country?: string; server?: string }
          verdicts?: { overall?: { score?: number; malicious?: boolean; categories?: string[] } }
          screenshot?: string
          _id?: string
        }>
      }
...
// ──────────────────────────────────────────────────────────────────
async function lookupIPInfo(ip: string) {
  try {
...
// ──────────────────────────────────────────────────────────────────
async function lookupRIPE(ip: string) {
  try {
  ...
  try {
    ...
    for (const report of (d?.reports ?? []).slice(0, 5)) {
      for (const cat of report.categories ?? []) {
        if (CATEGORIES[cat]) recentCategories.add(CATEGORIES[cat])
      }
    }
...
// ──────────────────────────────────────────────────────────────────
async function analysePhone(query: string) {
  ...
  if (ukMobile) {
    ...
    if (['740','741','742','743','744','745','746'].includes(prefix7)) carrier_note = 'Likely: O2 UK'
    else if (['770','771','772','773','774','775','776','777','778','779'].includes(prefix7)) carrier_note = 'Likely: Vodafone UK'
    else if (['730','731','732','733','734','735','736','737','738','739'].includes(prefix7)) carrier_note = 'Likely: EE UK'
    else if (['750','751','752','753','754','755','756','757','758','759'].includes(prefix7)) carrier_note = 'Likely: Three UK'
    else carrier_note = 'UK mobile — carrier not determinable without live lookup'
  } else if (ukLandline) {
  ...
  let numverifyResult: Record<string, unknown> | null = null
  ...
  if (numverifyKey) {
    try {
      ...
      if (nvRes.ok) {
        const nvJson = await nvRes.json() as {
          valid?: boolean
          carrier?: string
          line_type?: string
          location?: string
...
// ──────────────────────────────────────────────────────────────────
async function analyseEmail(query: string) {
  const flags: string[] = []
  ...
  try {
    ...
    if (mcRes.ok) {
      const mcJson = await mcRes.json() as {
        disposable?: boolean
        mx?: boolean
        domain?: string
      }
  ...
  let hibpResult: Record<string, unknown> | null = null
  ...
  if (hibpKey) {
    try {
      const hibpRes = await fetchWithTimeout(
        ...
        { headers: { 'hibp-api-key': hibpKey, 'User-Agent': 'INVESTiGATOR/1.0' } }
      )
      if (hibpRes.status === 404) {
        hibpResult = { breached: false, breach_count: 0 }
      } else if (hibpRes.ok) {
        const breaches = await hibpRes.json() as Array<{ Name: string; BreachDate: string; DataClasses: string[] }>
        hibpResult = {
          breached: true,
  ...
  for (const s of domainSources) {
    if (s && typeof s === 'object' && 'flags' in s) {
      flags.push(...(s as { flags: string[] }).flags)
    }
  }
...
router.post('/', investigateLimiter, async (req: Request, res: Res) => {
  const parsed = InvestigateSchema.safeParse(req.body)
  if (!parsed.success) {
    res.status(400).json({
      error: parsed.error.issues[0].message,
      valid_types: QUERY_TYPES,
      _legal: LEGAL.API_FOOTER,
    })
    return
  }
  ...
  try {
    ...
    switch (type as QueryType) {
      ...
      case 'company': {
        ...
        if (chResult && typeof chResult === 'object' && 'results' in chResult) {
          const r = chResult as { results?: Array<{ company_number?: string }> }
          ...
          if (firstCo?.company_number) {
            ...
            if (filings.status === 'fulfilled' && filings.value) {
              ...
              if (filings.value.flags?.length) allFlags.push(...filings.value.flags)
            }
          }
        }
        ...
        for (const s of sources) {
          if (s && typeof s === 'object' && 'flags' in s) {
            allFlags.push(...(s as { flags: string[] }).flags)
          }
        }
        break
      }
      ...
      case 'phone': {
        ...
        if (result && typeof result === 'object' && 'flags' in result) {
          allFlags.push(...(result as { flags: string[] }).flags)
        }
        break
      }
    ...
    res.json({
      ...
      sources,
      fair_use: LEGAL.FAIR_USE_POLICY,
      _legal: LEGAL.API_FOOTER,
    })

  } catch (err) {
    console.error('[investigate] handler error:', err)
    res.status(500).json({ error: 'Internal server error', _legal: LEGAL.API_FOOTER })
  }
})
